# %% [markdown]
# # AIMO E11a — controlled multi-view drift + deployed V6 control
#
# Select **GPU T4 x2** and enable Internet.  This notebook is a fail-closed
# experiment, not a submission builder.  Stage A must pass before Stage B
# generates any independent labels.
...
NOTEBOOK_VERSION = "e11a-rrb-multiview-deployed-v6-control-1"
...
PROBE_VIEWS = ["original", "flatten", "irrelevant", "copied_wrapper"]
...
WORKING_ROOT = Path("/kaggle/working/aimo_e11a_rrb_multiview")
TEMP_ROOT = Path("/kaggle/temp/aimo_e11a_rrb_multiview")
...
FINAL_ZIP = Path("/kaggle/working/aimo_e11a_rrb_multiview_results.zip")
...
def gpu_preflight() -> dict:
    ...
    if len(devices) != 2 or any("T4" not in x["name"].upper() for x in devices):
        raise RuntimeError(f"E11a requires exactly Kaggle T4 x2; found {devices}")
...
def probe_view(problem: str, view: str) -> str:
    ...
    if view == "irrelevant":
        return (
            problem
            + "\n\nFormatting note: The following sentence is irrelevant to the mathematics "
            "and must be ignored: A blue notebook rests on a wooden table."
        )
    if view == "copied_wrapper":
        return "A student copied the following problem verbatim. Solve the copied problem.\n\n" + problem
...
def feature_fingerprint(row: dict, view: str) -> str:
    payload = {
        "schema": "e11a_selected_hidden_states_v1",
        "model_id": MODEL_ID,
        "model_revision": MODEL_REVISION,
        "system_prompt": OFFICIAL_SOLVE_PROMPT,
        "problem_id": row["problem_id"],
        "view": view,
        "view_text": probe_view(row["original_problem"], view),
        "layers": LAYERS,
...
def extract_all_views(tokenizer, model, split_rows: list[tuple[str, list[dict]]]) -> dict:
    import numpy as np
    import torch
        ...
        for row in rows:
            for view in PROBE_VIEWS:
                ...
                if not feature_cache_valid(path, digest):
                    jobs.append((split, row, view, digest, path))
    total = sum(len(rows) for _, rows in split_rows) * len(PROBE_VIEWS)
    print(f"Feature extraction: {total - len(jobs)}/{total} cached; {len(jobs)} pending", flush=True)
    started = time.time()
    for number, (split, row, view, digest, path) in enumerate(jobs, start=1):
        user_text = probe_view(row["original_problem"], view)
        prompt = build_chat_prompt(tokenizer, user_text, OFFICIAL_SOLVE_PROMPT)
        ...
        if matrix.shape != (len(LAYERS), 4096) or not np.isfinite(matrix).all():
            raise RuntimeError(f"invalid hidden states for {split}/{row['problem_id']}/{view}")
    ...
    return {"total_jobs": total, "new_jobs": len(jobs), "cached_jobs": total - len(jobs)}


def load_view_cube(split: str, rows: list[dict]):
    import numpy as np
    ...
    for row in rows:
        ...
        for view in PROBE_VIEWS:
            ...
            with np.load(path, allow_pickle=False) as data:
                per_view.append(data["hidden_states"].astype(np.float32))
    ...
    if cube.shape != (len(rows), len(PROBE_VIEWS), len(LAYERS), 4096):
        raise RuntimeError(f"unexpected view cube shape: {cube.shape}")
    return cube, token_counts


def make_multiview_features(cube):
    import numpy as np

    original = cube[:, 0, :, :].astype(np.float64)
    features = []
    names = []
    for view_index, view in enumerate(PROBE_VIEWS[1:], start=1):
        changed = cube[:, view_index, :, :].astype(np.float64)
        dot = np.sum(original * changed, axis=-1)
        original_norm = np.linalg.norm(original, axis=-1)
        changed_norm = np.linalg.norm(changed, axis=-1)
        cosine = 1.0 - dot / np.maximum(original_norm * changed_norm, 1e-12)
        relative_l2 = np.linalg.norm(changed - original, axis=-1) / np.maximum(original_norm, 1e-12)
        for metric_name, values in (("cosine", cosine), ("relative_l2", relative_l2)):
            summaries = {
                "mean": values.mean(axis=1),
                "late_mean": values[:, -3:].mean(axis=1),
                "max": values.max(axis=1),
            }
            for summary_name, vector in summaries.items():
                features.append(vector)
                names.append(f"{view}_{metric_name}_{summary_name}")
    matrix = np.stack(features, axis=1).astype(np.float32)
    if matrix.shape != (cube.shape[0], 18) or not np.isfinite(matrix).all():
        raise RuntimeError(f"invalid E11a feature matrix: {matrix.shape}")
    return matrix, names
...
def stage_a_validation(
    ...
    official_x, names = make_multiview_features(official_cube)
    external_x, external_names = make_multiview_features(external_cube)
    if names != feature_names or external_names != feature_names:
        raise RuntimeError("feature-name mismatch")

    candidate_probability, external_candidate_probability, folds = candidate_oof(
        official_x, y, groups, external_x
    )
    ...
    external_v6_fraction = deployed_v6_vote_fraction(
        external_cube[:, 0, :, :], deployed_v6_artifact
    )
    ...
    for row, candidate_p, v6_p in zip(
        external_rows, external_candidate_probability, external_v6_fraction, strict=True
    ):
        predictions.append(
            {
                "problem_id": row["problem_id"],
...
def main() -> None:
    ...
    extraction_audit = extract_all_views(
        tokenizer, model, [("official", official_rows), ("external", external_rows)]
    )
    official_cube, official_token_counts = load_view_cube("official", official_rows)
    external_cube, external_token_counts = load_view_cube("external", external_rows)
    _, feature_names = make_multiview_features(official_cube[:1])
    stage_a, frozen_predictions = stage_a_validation(
        official_rows,
        official_cube,
        external_rows,
        external_cube,
    ...
    manifest = {
        ...
        "max_external_token_count": int(np.max(external_token_counts)),
    ...
    if not stage_a["passed"]:
        payload = {
            "experiment": "E11a_controlled_multiview_latent_drift",
            "status": "FAIL_STAGE_A",
            "stage_a": stage_a,
            "stage_b": None,
    ...
    finalize(payload)
    print(
        "E11a passed both gates; this permits a separate submission-engineering step."
        if stage_b["passed"]
