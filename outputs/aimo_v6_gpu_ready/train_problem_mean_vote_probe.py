#!/usr/bin/env python3
"""E7: fixed equal vote over problem-token mean representations."""
...
def audit_internals(internals: Path, rows: list[dict[str, str]]) -> dict:
    ...
    if not manifest_path.is_file():
        raise FileNotFoundError("problem-mean internals are missing manifest.json")
    ...
    if views != {"problem_mean"}:
        mismatches["metadata.extraction_view"] = {
            "expected": ["problem_mean"],
            "actual": sorted(str(value) for value in views),
        }
    if mismatches:
        raise RuntimeError(f"problem-mean internals audit failed: {mismatches}")
    return manifest
...
def write_outputs(
    ...
    with (output_dir / "oof_predictions.csv").open(
        ...
        fields = [
            ...
            "dataset_id",
            "label",
            "positive_problem_mean_vote_fraction",
            "prediction",
            "v6_prediction",
            "changed_vs_v6",
            "correct",
            ...
            writer.writerow(
                {
                    ...
                    "label": bool(label),
                    "positive_problem_mean_vote_fraction": f"{fraction:.12g}",
    ...
    report = f"""# E7 problem-token mean representation vote
...
def main() -> int:
    ...
    if args.layers != EXPECTED_LAYERS:
        raise ValueError("E7 layers differ from the preregistered sequence")
    ...
    result = {
        "schema_version": 1,
        "experiment": "E7_problem_token_mean_vote",
        "selection_scope": "fixed_problem_mean_layer_vote_grouped",
