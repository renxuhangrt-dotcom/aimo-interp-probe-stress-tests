#!/usr/bin/env python3
"""E6: fixed equal vote over adjacent-layer representation deltas."""
...
EXPECTED_ENDPOINTS = [4, 8, 12, 16, 20, 24, 28, 32, 36]


def build_delta_layers(
    raw_layers: dict[int, np.ndarray], endpoints: list[int]
) -> dict[int, np.ndarray]:
    result: dict[int, np.ndarray] = {}
    for high in endpoints:
        low = high - 4
        if high not in raw_layers or low not in raw_layers:
            raise ValueError(f"missing raw layer for delta {high}-{low}")
        high_values = np.asarray(raw_layers[high], dtype=np.float32)
        low_values = np.asarray(raw_layers[low], dtype=np.float32)
        if high_values.shape != low_values.shape:
            raise ValueError(f"shape mismatch for delta {high}-{low}")
        delta = np.asarray(high_values - low_values, dtype=np.float32)
        if not np.isfinite(delta).all():
            raise ValueError(f"non-finite values in delta {high}-{low}")
        result[high] = delta
    return result
...
def write_outputs(
            ...
            writer.writerow(
                {
                    ...
                    "label": bool(label),
                    "positive_delta_vote_fraction": f"{fraction:.12g}",
    ...
    report = f"""# E6 adjacent-layer representation-change vote
...
def main() -> int:
    ...
    parser.add_argument("--endpoints", type=int, nargs="+", default=EXPECTED_ENDPOINTS)
    ...
    if args.endpoints != EXPECTED_ENDPOINTS:
        raise ValueError("E6 endpoints differ from the preregistered sequence")
    ...
    requested_raw = sorted({0, *args.endpoints, *(value - 4 for value in args.endpoints)})
    raw_layers = base.load_layers(args.internals, requested_raw)
    layers = build_delta_layers(raw_layers, args.endpoints)
    ...
    result = {
        "schema_version": 1,
        "experiment": "E6_adjacent_layer_delta_vote",
        "selection_scope": "fixed_layer_delta_problem_grouped",
        "interpretation": "sequential_exploratory",
        "deltas": [f"{high}-{high - 4}" for high in args.endpoints],
